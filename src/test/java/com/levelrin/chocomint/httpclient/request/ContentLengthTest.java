...
final class ContentLengthTest {

    @Test
    void shouldAddContentLength() throws IOException {
        final AtomicReference<String> actual = new AtomicReference<>();
        try (FakeOutputStream output = new FakeOutputStream().with(data -> {
            actual.set(new String(data, StandardCharsets.UTF_8));
        })) {
            new ContentLength(new byte[] {1}).accept(output);
        }
        MatcherAssert.assertThat(
            "Should add Content-Length header.",
            actual.get(),
            CoreMatchers.equalTo("Content-Length: 1\r\n")
        );
    }
